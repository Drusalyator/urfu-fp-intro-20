...
inHowManyYearsChinaWins = 1 + length (takeWhile (\(x, y) -> x < y) (zip chinaGDP usaGDP))
-- inHowManyYearsChinaWins = length (takeWhile (\(x, y) -> x < y) (zip chinaGDP usaGDP))
...
isSameCountry :: String -> Country -> Bool
isSameCountry currentName (Country name _) = currentName  == name

getSickCount :: Country -> Integer
getSickCount (Country _ count) = count

getSickSum :: String -> [Country] -> Integer
getSickSum countryName events = sum (map getSickCount (filter (isSameCountry countryName) events))

stat :: [Country] -> [Country]
stat events = map (\(Country name _) -> Country name (getSickSum name events)) allCountries
