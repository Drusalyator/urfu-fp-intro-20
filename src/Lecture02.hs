...
map f n = foldr (\x xn -> f x :. xn) Nil n
...
filter f n = foldr (\x xn -> if f x then x :. xn else xn) Nil n
...
toListH :: List a -> [a]
toListH Nil = []
toListH (n :. s) = n : toListH s

-- И обратно
fromListH :: [a] -> List a
fromListH [] = Nil
fromListH (n : s) = n :. fromListH s
...
-}
intercalate :: [a] -> [[a]] -> [a]
intercalate sep ls = P.foldr (\s k -> if null k then s else s ++ sep ++ k) [] ls
